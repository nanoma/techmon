...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        enemyTimer = NSTimer.scheduledTimerWithTimeInterval(NSTimeInterval(enemy.speed), target: self, selector:
            "enemyAttack", userInfo: nil, repeats: true)
        enemyTimer.fire()
    }
    ...
    @IBAction func playerAttack(){
        ...
        if enemy.currentHP <= 0.0 {
           fnishBattle(enemyImageView, winPlayer: true)
        }
    }
    ...
    func enemyAttack(){
        ...
        if player.currentHP <= 0.0 {
            fnishBattle(playerImageView, winPlayer: false)
        }
    }
    
    func fnishBattle(vanishImageView: UIImageView, winPlayer: Bool){
    ...
    //MARK: Cure
    func cureHP(){
        timer = NSTimer.scheduledTimerWithTimeInterval(0.1, target: self, selector: "updateHPValue", userInfo: nil,
        repeats:  true)
        timer.fire()
    }
